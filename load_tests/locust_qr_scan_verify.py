...
def create_test_user():
    ...
    user, _ = User.objects.get_or_create(
        username="qrload_customer_001",
        defaults={"email": "qrload_customer_001@test.local"},
    )
...
class QRScanVerifyUser(HttpUser):
    ...
    def on_start(self):
        ...
        # Locust user start ayyaka test branch + qr_code offer ready cheyyi
    ...
    def scan_verify_valid_qr_code_flow(self):
        ...
        # IMPORTANT:
        # Previous response can set session cookies.
        # Clear them so each request uses the fresh test user session.
        self.client.cookies.clear()

        # Attach Django auth + csrf cookies to this Locust client session
        ...
        with self.client.post(
            ...
            headers=headers,
            name="POST /qrg/scan-verify/ valid qr_code",
            catch_response=True,
        ) as response:
            try:
                data = response.json()
