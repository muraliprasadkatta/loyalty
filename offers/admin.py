...
@admin.register(UserPendingVisitAttempt)
class UserPendingVisitAttemptAdmin(admin.ModelAdmin):
    list_display = (
        "id","user","branch","method","state","desk","qr_token","yashpin","started_at","updated_at",)
    
    list_filter = ("state", "method", "branch", "started_at", "updated_at")
    search_fields = ("user__username","user__email","branch__name","desk","qr_token__token",)

    autocomplete_fields = ("user", "branch", "qr_token", "yashpin")
    ordering = ("-started_at",)
    ...
    fieldsets = (
        ("Core", { "fields": ("user","branch","method","state","desk","note",)}),
