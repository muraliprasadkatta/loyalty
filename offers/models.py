...
class UserPendingVisitAttempt(models.Model):
    ...
    STATE_CHOICES = [
        (STATE_STARTED, "Started"),
        (STATE_AWAITING_BRANCH, "Awaiting branch verification"),
        (STATE_COMPLETED, "Completed"),
        (STATE_EXPIRED, "Expired"),
        (STATE_CANCELLED, "Cancelled"),
    ]
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="pending_visit_attempts")
    branch = models.ForeignKey("Branch", on_delete=models.CASCADE, related_name="pending_visit_attempts")
    qr_token = models.ForeignKey("QRToken", on_delete=models.SET_NULL, null=True, blank=True, related_name="pending_attempts")
    yashpin = models.ForeignKey("YashPin", on_delete=models.SET_NULL, null=True, blank=True, related_name="pending_attempts")

    method = models.CharField(max_length=16, choices=METHOD_CHOICES)
    state = models.CharField(max_length=32, choices=STATE_CHOICES, default=STATE_STARTED)
